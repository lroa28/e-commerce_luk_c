...
router.post('/loginjwt',async(req,res)=>{
    let user = await usersService.findOne({email:req.body.email});//corr si el usu existe
    if(!user) return res.status(400).send({status:"error",error:"User doesn't exist"});//si si existe el usu 
    if(!isValidPassword(user,req.body.password)) return res.status(400).send({status:"error",error:"Invalid Password"});
    
    //antes que creabamos la session, ahora creamos una firma de token y le paso la clave
