...
Desarrollar una función ‘mostrarLetras’ que reciba un string como parámetro y permita mostrar una vez por segundo cada 
uno de sus caracteres. 
Al finalizar, debe invocar a la siguiente función que se le pasa también  como parámetro: 
const fin = () => console.log('terminé')
Realizar tres llamadas a ‘mostrarLetras’ con el mensaje ‘¡Hola!’ y demoras de 0, 250 y 500 mS 
...
const date = new Date();
let contenido = `${date.toLocaleDateString()} - ${date.toTimeString()}`;

fs.writeFileSync(fileName, contenido);

try {
    let paraMostrar = fs.readFileSync(fileName);
    console.log(paraMostrar.toString())
} catch(err) {
    console.log("Error intentando leer el archivo");
    throw new Error('Hubo un error al intentar leer el archivo')
}


//---------- SLIDE #52 Lectura y escritura de archivos --------//
/* Se vio vio en el 1re after */
...
Ayuda:
Para el Punto C considerar usar JSON.stringify(info, null, 2) para preservar el formato de representación del objeto en el 
archivo (2 representa en este caso la cantidad de espacios de indentación usadas al representar el objeto como string).
En la diapositiva 54 tienen una forma para que el formato dentro del archivo sea "fácilmente legible", en el JSON.stringify 
dben pasarle unos parámetros más
Ejemplo: JSON.stringify(arrayDeDatos, null, 2)
tengo duda con el ID, por ejemplo, si decidimos eliminar el ID 1 y ya tenemos otros 3 productos hasta el ID 4, 
el siguiente producto que agreguemos debe adquirir el número del ID 1 o puede continuar aumentando a pesar de que hayamos 
perdido el ID 1?
...
//leemos el archivo
fs.readFile('./package.json', 'utf-8', (error, contenido) => { //contenido: lo que me vino de haber hecho el readFile
    let fileSize;
    if (error) {
...
//muestra el contenido en formato string
        const info = { //objeto anonimo sin clase
            contenidoStr: contenido, // version string cadena de caracteres del package.json
            contenidoObj: JSON.parse(contenido), //genera un obj Jsonmetodo .parse nativo de JS entender lo que vino de un string e interpretarlo
            size: fileSize
        }
        console.log(info);

//muestra el archivo como un objeto Json
        const infoString = JSON.stringify(info.contenidoObj, null, 2 ); //volver un string/crear un string, el 2 son los espacios y comas.
        
//sobreescribimos el archivo info.txt, como una cadena de caracteres
...
Ayuda:
Para el punto 3 considerar usar JSON.stringify(info.contenidoObj, null,2) para preservar el formato de representación 
del objeto en el archivo.*/
...
fs.readFile('./package.json', 'utf-8', (error, contenido) => {
    if (error) {
        ...
        const info = {
            ...
            contenidoStr: JSON.parse(contenido)
        }
        console.log(info);
    }
})
